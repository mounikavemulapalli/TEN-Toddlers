...
export default function Blog({ searchedKeyword, setSelectedTitle }) {
  ...
  return (
    <div>
      <div id="container">
        {searchedKeyword == '' ? <h2 id='heading'>TEN-Toddlers PARENTING BLOGS</h2> : <h2 id="search-result-heading">SEARCH RESULTS FOR: {searchedKeyword.toUpperCase()}</h2>}
        <div id='line'></div>
...
const handleReplyCheckboxChange=(e)=>{
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const response = await axios.get(`https://uptodd-29rq.onrender.com/api/comments/${encodeURIComponent(title)}`);
...
const latestComment=comments[comments.length - 1] //Get the latest comment if available
  ...
  const handleSubmit=(e)=>{
    ...
    axios.post(`https://uptodd-29rq.onrender.com/api/comments/${encodeURIComponent(title)}`,newComment)
    .then(response=>{
...
const handleDelete=async(commentId)=>{
  try {
    await axios.delete(`https://uptodd-29rq.onrender.com/api/comments/${encodeURIComponent(title)}/${commentId}`);
...
//if approved, proceed with submitting the reply
  try {
    ...
    // Only write this logic if you want to display reply text
     const response=await axios.post(`https://uptodd-29rq.onrender.com/api/comments/${encodeURIComponent(title)}/${commentId}/reply`,{
      author:replyAuthor,
      email:replyEmail,
      text:replyText,
     });
...
const handleDeleteReply = async (commentId, replyId) => {
  ...
  try {
    const response = await axios.delete(`https://uptodd-29rq.onrender.com/api/comments/${encodeURIComponent(title)}/${commentId}/reply/${replyId}`);
